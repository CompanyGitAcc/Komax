report 50037 "CN Purchase Order"
{
    WordLayout = './Layout/CNPurchaseOrder.docx';
    Caption = 'Purchase - Confirmation';
    DefaultLayout = Word;
    ...
    dataset
    {
        dataitem("Purchase Header"; "Purchase Header")
        {
            ...
            //++Harvey
            column(Forwarder; Forwarder)
            {
            }
            column(Way_of_Dispatch; "Way of Dispatch")
            {
            }
            column(Partial_Delivery; "Partial Delivery")
            {
            }
            column(Requested_Receipt_Date; Format("Requested Receipt Date", 0, '<Year4>-<Month,2>-<Day,2>'))
            {
            }
            column(CompanyInfoName; CompanyInfo."Name 2")
            {
            }
            column(CompanyInfoAddress; CompanyInfo."Address 2")
            {
            }
            ...
            column(Pay_to_Name; "Pay-to Name 2")
            {
            }
            column(Pay_to_Address; "Pay-to Address 2")
            {
            }
            ...
            column(Buy_from_Vendor_Name; "Buy-from Vendor Name 2")
            {
            }
            ...
            column(Ship_to_Name; "Ship-to Name 2")
            {
            }
            column(Ship_to_Address; "Ship-to Address 2")
            {
            }
            column(CustomShiptoAddr; CustomShiptoAddr)
            {
            }
            column(Ship_to_City; CustomShiptoCity)
            {
            }
            column(Ship_to_Post_Code; CustomShiptoPostCode)
            {
            }
            ...
            dataitem("Purchase Line"; "Purchase Line")
            {
                ...
                column(Vendor_Item__; "Vendor Item #")
                {
                }
                ...
                column(Expected_Receipt_Date; Format("Expected Receipt Date", 0, '<Year4>-<Month,2>-<Day,2>'))
                {
                }
                ...
                begin
                    ...
                    if Item.get("No.") then begin
                        ...
                        //extend txt
                        ExtendTxt.Reset();
                        ExtendTxt.SetRange("Table Name", ExtendTxt."Table Name"::Item);
                        ExtendTxt.SetRange("No.", Item."No.");
                        if ExtendTxt.FindFirst() then
                            ItemDescription := ItemDescription + ' ' + ExtendTxt.Text + '/' + ExtendTxt.Index;
                    ...
                    if ItemVendor.FindFirst() then
                        ItemReferenceNo := ItemVendor."Vendor Item No."
                    else
                        ItemReferenceNo := '';
                end;
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(ShipmentMethodDesc; ShipmentMethod.Code)
                    {
                    }
            ...
            begin
                ...
                if "Prices Including VAT" = true then begin
                    UnitPriceLbl := '含税单价';
                    LineAmountLbl := '含税行金额 (' + CurrencyCode + ')';
                end else begin
                    UnitPriceLbl := '未税单价';
                    LineAmountLbl := '未税行金额(' + CurrencyCode + ')';
                end;

                if "Customs Supervision" = true then begin
                    CustomShiptoAddr := CompanyInfo."Customs Ship-to Address";
                    CustomShiptoCity := '';
                    CustomShiptoPostCode := '';
                end else begin
                    CustomShiptoAddr := "Ship-to Address";
                    CustomShiptoCity := "Ship-to City";
                    CustomShiptoPostCode := "Ship-to Post Code";
                end;
                "Ship-to Name" := 'Komax Shanghai';
            end;
