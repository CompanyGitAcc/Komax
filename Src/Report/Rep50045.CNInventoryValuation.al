report 50045 "CN Inventory Valuation"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Layout/InventoryValuation.rdlc';
        ...
        dataitem(Item; Item)
        {
            ...
            begin
                ...
                IF (ExportToExcel = TRUE) THEN BEGIN
                    RowNo := 0;
                    RowNo := RowNo + 1;
                    EnterCell(RowNo, 1, 'Inventory Valuation Report', TRUE, FALSE, FALSE);
                    RowNo := RowNo + 1;
                    EnterCell(RowNo, 1, Format("Date Filter"), TRUE, FALSE, FALSE);
                    RowNo := RowNo + 1;
                    EnterCell(RowNo, 1, '', TRUE, FALSE, FALSE);

                    RowNo := RowNo + 1;
                    EnterCell(RowNo, 1, 'Posting Group', FALSE, FALSE, FALSE);
                    EnterCell(RowNo, 2, 'Item No.', FALSE, FALSE, FALSE);
                    EnterCell(RowNo, 3, 'Description', FALSE, FALSE, FALSE);
                    EnterCell(RowNo, 4, 'Base UOM', FALSE, FALSE, FALSE);
                    EnterCell(RowNo, 5, 'Starting-QTY', FALSE, FALSE, FALSE);
                    EnterCell(RowNo, 6, 'Starting-Value', FALSE, FALSE, FALSE);
                    EnterCell(RowNo, 7, 'Increase-QTY', FALSE, FALSE, FALSE);
                    EnterCell(RowNo, 8, 'Increase-Value', FALSE, FALSE, FALSE);
                    EnterCell(RowNo, 9, 'Descrease-QTY', FALSE, FALSE, FALSE);
                    EnterCell(RowNo, 10, 'Descrease-Value', FALSE, FALSE, FALSE);
                    EnterCell(RowNo, 11, 'Ending-QTY', FALSE, FALSE, FALSE);
                    EnterCell(RowNo, 12, 'Ending-Value', FALSE, FALSE, FALSE);
                END;
            end;
            //--ALF
            ...
            begin
                ...
                //++ALF
                IF (ExportToExcel = TRUE) AND
                ((StartingExpectedQty <> 0) or (IncreaseExpectedQty <> 0) or (DecreaseExpectedQty <> 0)
                or (StartingInvoicedQty <> 0) or (IncreaseExpectedQty <> 0) or (DecreaseExpectedQty <> 0))
                                            THEN BEGIN
                    RowNo := RowNo + 1;
                    EnterCell(RowNo, 1, FORMAT(Item."Inventory Posting Group"), FALSE, FALSE, FALSE);
                    EnterCell(RowNo, 2, FORMAT('"' + Item."No."), FALSE, FALSE, FALSE);
                    EnterCell(RowNo, 3, FORMAT(Item.Description), FALSE, FALSE, FALSE);
                    EnterCell(RowNo, 4, FORMAT(Item."Base Unit of Measure"), FALSE, FALSE, FALSE);

                    if ((StartingExpectedQty <> 0) or (IncreaseExpectedQty <> 0) or (DecreaseExpectedQty <> 0)
                        or (StartingInvoicedQty <> 0) or (IncreaseInvoicedQty <> 0) or (DecreaseInvoicedQty <> 0))
                        and
                        Not ((StartingExpectedQty = StartingInvoicedQty) and (IncreaseExpectedQty = IncreaseInvoicedQty) and (DecreaseExpectedQty = DecreaseInvoicedQty)) then begin
                        EnterCell(RowNo, 5, FORMAT(StartingExpectedQty), FALSE, FALSE, FALSE);
                        EnterCell(RowNo, 6, FORMAT(StartingExpectedValue), FALSE, FALSE, FALSE);
                        EnterCell(RowNo, 7, FORMAT(IncreaseExpectedQty), FALSE, FALSE, FALSE);
                        EnterCell(RowNo, 8, FORMAT(IncreaseExpectedValue), FALSE, FALSE, FALSE);
                        EnterCell(RowNo, 9, FORMAT(DecreaseExpectedQty), FALSE, FALSE, FALSE);
                        EnterCell(RowNo, 10, FORMAT(DecreaseExpectedValue), FALSE, FALSE, FALSE);
                        EnterCell(RowNo, 11, FORMAT(StartingExpectedQty + IncreaseExpectedQty - DecreaseExpectedQty), FALSE, FALSE, FALSE);
                        EnterCell(RowNo, 12, FORMAT(StartingExpectedValue + IncreaseExpectedValue - DecreaseExpectedValue), FALSE, FALSE, FALSE);

                    end else begin
                        EnterCell(RowNo, 5, FORMAT(StartingInvoicedQty), FALSE, FALSE, FALSE);
                        EnterCell(RowNo, 6, FORMAT(StartingInvoicedValue), FALSE, FALSE, FALSE);
                        EnterCell(RowNo, 7, FORMAT(IncreaseInvoicedQty), FALSE, FALSE, FALSE);
                        EnterCell(RowNo, 8, FORMAT(IncreaseInvoicedValue), FALSE, FALSE, FALSE);
                        EnterCell(RowNo, 9, FORMAT(DecreaseInvoicedQty), FALSE, FALSE, FALSE);
                        EnterCell(RowNo, 10, FORMAT(DecreaseInvoicedValue), FALSE, FALSE, FALSE);
                        EnterCell(RowNo, 11, FORMAT(StartingInvoicedQty + IncreaseInvoicedQty - DecreaseInvoicedQty), FALSE, FALSE, FALSE);
                        EnterCell(RowNo, 12, FORMAT(StartingInvoicedValue + IncreaseInvoicedValue - DecreaseInvoicedValue), FALSE, FALSE, FALSE);

                    end;

                    // if StartingExpectedQty <> 0 then begin
                    //     EnterCell(RowNo, 5, FORMAT(StartingExpectedQty), FALSE, FALSE, FALSE);
                    //     EnterCell(RowNo, 6, FORMAT(StartingExpectedValue), FALSE, FALSE, FALSE);
                    // end else begin
                    //     EnterCell(RowNo, 5, FORMAT(StartingInvoicedQty), FALSE, FALSE, FALSE);
                    //     EnterCell(RowNo, 6, FORMAT(StartingInvoicedValue), FALSE, FALSE, FALSE);
                    // end;
                    // if IncreaseExpectedQty <> 0 then begin
                    //     EnterCell(RowNo, 7, FORMAT(IncreaseExpectedQty), FALSE, FALSE, FALSE);
                    //     EnterCell(RowNo, 8, FORMAT(IncreaseExpectedValue), FALSE, FALSE, FALSE);
                    // end else begin
                    //     EnterCell(RowNo, 7, FORMAT(IncreaseInvoicedQty), FALSE, FALSE, FALSE);
                    //     EnterCell(RowNo, 8, FORMAT(IncreaseInvoicedValue), FALSE, FALSE, FALSE);
                    // end;
                    // if DecreaseExpectedQty <> 0 then begin
                    //     EnterCell(RowNo, 9, FORMAT(DecreaseExpectedQty), FALSE, FALSE, FALSE);
                    //     EnterCell(RowNo, 10, FORMAT(DecreaseExpectedValue), FALSE, FALSE, FALSE);
                    // end else begin
                    //     EnterCell(RowNo, 9, FORMAT(DecreaseInvoicedQty), FALSE, FALSE, FALSE);
                    ...
                    //     EnterCell(RowNo, 11, FORMAT(StartingExpectedQty), FALSE, FALSE, FALSE);
                    //     EnterCell(RowNo, 12, FORMAT(StartingExpectedValue), FALSE, FALSE, FALSE);
                    // end else begin
                    //     EnterCell(RowNo, 11, FORMAT(StartingInvoicedQty), FALSE, FALSE, FALSE);
                    //     EnterCell(RowNo, 12, FORMAT(StartingInvoicedValue), FALSE, FALSE, FALSE);
                    // end;
                END;
                //--ALF
                IsHandled := false;
    ...
    begin
        IF ExportToExcel = TRUE THEN BEGIN
            TempExcelBuffer.CreateNewBook('Inventory Valuation');
            //FillExcelBuffer(TempExcelBuf);
            TempExcelBuffer.WriteSheet('Sheet1', CompanyName(), UserId());
            TempExcelBuffer.CloseBook();
            TempExcelBuffer.SetFriendlyFilename('Inventory Valuation');
